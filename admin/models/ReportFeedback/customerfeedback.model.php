...
class CustomerFeedback extends Model implements JsonSerializable{
	...
	public static function delete($id){
		global $db,$tx;
		$db->query("delete from {$tx}customer_feedbacks where id={$id}");
	}
	...
	public static function count($criteria=""){
		global $db,$tx;
		$result =$db->query("select count(*) from {$tx}customer_feedbacks $criteria");
	...
	static function get_last_id(){
		global $db,$tx;
		$result =$db->query("select max(id) last_id from {$tx}customer_feedbacks");
		$customerfeedback =$result->fetch_object();
...
";
	...
	static function html_select($name="cmbCustomerFeedback"){
		...
		$result =$db->query("select id,name from {$tx}customer_feedbacks");
		while($customerfeedback=$result->fetch_object()){
	...
	static function html_table($page = 1,$perpage = 10,$criteria="",$action=true){
		...
		$count_result =$db->query("select count(*) total from {$tx}customer_feedbacks $criteria ");
