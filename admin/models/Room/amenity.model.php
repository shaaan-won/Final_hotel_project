...
class Amenity extends Model implements JsonSerializable{
	...
	public function set($id,$name,$description,$created_at){
		$this->id=$id;
		$this->name=$name;
	...
	public function save(){
		global $db,$tx;
		$db->query("insert into {$tx}amenities(name,description,created_at)values('$this->name','$this->description','$this->created_at')");
		return $db->insert_id;
	}
	public function update(){
		global $db,$tx;
		$db->query("update {$tx}amenities set name='$this->name',description='$this->description',created_at='$this->created_at' where id='$this->id'");
	}
	...
	public static function all(){
		global $db,$tx;
		$result=$db->query("select id,name,description,created_at from {$tx}amenities");
		$data=[];
	...
	public static function pagination($page=1,$perpage=10,$criteria=""){
		global $db,$tx;
		$top=($page-1)*$perpage;
		$result=$db->query("select id,name,description,created_at from {$tx}amenities $criteria limit $top,$perpage");
		$data=[];
		while($amenity=$result->fetch_object()){
			$data[]=$amenity;
		}
	...
	public static function find($id){
		global $db,$tx;
		$result =$db->query("select id,name,description,created_at from {$tx}amenities where id='$id'");
...
";
	...
	static function html_table($page = 1,$perpage = 10,$criteria="",$action=true){
		...
		if($action){
			$html.="<tr><th>Id</th><th>Name</th><th>Description</th><th>Created At</th><th>Action</th></tr>";
		}else{
			$html.="<tr><th>Id</th><th>Name</th><th>Description</th><th>Created At</th></tr>";
		}
		while($amenity=$result->fetch_object()){
			$action_buttons = "";
			if($action){
			...
			$html.="<tr><td>$amenity->id</td><td>$amenity->name</td><td>$amenity->description</td><td>$amenity->created_at</td> $action_buttons</tr>";
		}
		$html.="</table>";
