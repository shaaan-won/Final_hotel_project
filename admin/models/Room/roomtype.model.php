...
class RoomType extends Model implements JsonSerializable{
	...
	public function set($id,$name,$description,$photo,$created_at){
		$this->id=$id;
		$this->name=$name;
	...
	public function save(){
		global $db,$tx;
		$db->query("insert into {$tx}room_types(name,description,photo,created_at)values('$this->name','$this->description','$this->photo','$this->created_at')");
		return $db->insert_id;
	}
	public function update(){
		global $db,$tx;
		$db->query("update {$tx}room_types set name='$this->name',description='$this->description',photo='$this->photo',created_at='$this->created_at' where id='$this->id'");
	}
	...
	public static function all(){
		global $db,$tx;
		$result=$db->query("select id,name,description,photo,created_at from {$tx}room_types");
		$data=[];
	...
	public static function pagination($page=1,$perpage=10,$criteria=""){
		global $db,$tx;
		$top=($page-1)*$perpage;
		$result=$db->query("select id,name,description,photo,created_at from {$tx}room_types $criteria limit $top,$perpage");
		$data=[];
		while($roomtype=$result->fetch_object()){
			$data[]=$roomtype;
		}
	...
	public static function find($id){
		global $db,$tx;
		$result =$db->query("select id,name,description,photo,created_at from {$tx}room_types where id='$id'");
...
";
	...
	static function html_table($page = 1,$perpage = 10,$criteria="",$action=true){
		...
		if($action){
			$html.="<tr><th>Id</th><th>Name</th><th>Description</th><th>Photo</th><th>Created At</th><th>Action</th></tr>";
		}else{
			$html.="<tr><th>Id</th><th>Name</th><th>Description</th><th>Photo</th><th>Created At</th></tr>";
		}
		while($roomtype=$result->fetch_object()){
			$action_buttons = "";
			if($action){
			...
			$html.="<tr><td>$roomtype->id</td><td>$roomtype->name</td><td>$roomtype->description</td><td><img src='$base_url/img/$roomtype->photo' width='100' /></td><td>$roomtype->created_at</td> $action_buttons</tr>";
		}
		$html.="</table>";
