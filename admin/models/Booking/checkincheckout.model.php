...
class CheckinCheckout extends Model implements JsonSerializable{
	...
	public function set($id,$booking_id,$check_in_date,$check_out_date,$notes,$created_at,$updated_at){
		$this->id=$id;
	...
	public function save(){
		global $db,$tx;
		$db->query("insert into {$tx}checkin_checkouts(booking_id,check_in_date,check_out_date,notes,created_at,updated_at)values('$this->booking_id','$this->check_in_date','$this->check_out_date','$this->notes','$this->created_at','$this->updated_at')");
		return $db->insert_id;
	}
	public function update(){
		global $db,$tx;
		$db->query("update {$tx}checkin_checkouts set booking_id='$this->booking_id',check_in_date='$this->check_in_date',check_out_date='$this->check_out_date',notes='$this->notes',created_at='$this->created_at',updated_at='$this->updated_at' where id='$this->id'");
	}
	...
	public static function all(){
		global $db,$tx;
		$result=$db->query("select id,booking_id,check_in_date,check_out_date,notes,created_at,updated_at from {$tx}checkin_checkouts");
		$data=[];
	...
	public static function pagination($page=1,$perpage=10,$criteria=""){
		global $db,$tx;
		$top=($page-1)*$perpage;
		$result=$db->query("select id,booking_id,check_in_date,check_out_date,notes,created_at,updated_at from {$tx}checkin_checkouts $criteria limit $top,$perpage");
		$data=[];
		while($checkincheckout=$result->fetch_object()){
			$data[]=$checkincheckout;
		}
	...
	public static function find($id){
		global $db,$tx;
		$result =$db->query("select id,booking_id,check_in_date,check_out_date,notes,created_at,updated_at from {$tx}checkin_checkouts where id='$id'");
...
";
	...
	static function html_table($page = 1,$perpage = 10,$criteria="",$action=true){
		...
		if($action){
			$html.="<tr><th>Id</th><th>Booking Id</th><th>Check In Date</th><th>Check Out Date</th><th>Notes</th><th>Created At</th><th>Updated At</th><th>Action</th></tr>";
		}else{
			$html.="<tr><th>Id</th><th>Booking Id</th><th>Check In Date</th><th>Check Out Date</th><th>Notes</th><th>Created At</th><th>Updated At</th></tr>";
		}
		while($checkincheckout=$result->fetch_object()){
			$action_buttons = "";
			if($action){
			...
			$html.="<tr><td>$checkincheckout->id</td><td>$checkincheckout->booking_id</td><td>$checkincheckout->check_in_date</td><td>$checkincheckout->check_out_date</td><td>$checkincheckout->notes</td><td>$checkincheckout->created_at</td><td>$checkincheckout->updated_at</td> $action_buttons</tr>";
		}
		$html.="</table>";
	...
	static function html_row_details($id){
		...
		$html.="<tr><th>Booking Id</th><td>$checkincheckout->booking_id</td></tr>";
