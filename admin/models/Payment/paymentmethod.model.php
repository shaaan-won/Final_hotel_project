...
class PaymentMethod extends Model implements JsonSerializable{
	...
	public function save(){
		global $db,$tx;
		$db->query("insert into {$tx}payment_methods(name,created_at)values('$this->name','$this->created_at')");
		return $db->insert_id;
	}
	public function update(){
		global $db,$tx;
		$db->query("update {$tx}payment_methods set name='$this->name',created_at='$this->created_at' where id='$this->id'");
	}
	...
	public static function all(){
		global $db,$tx;
		$result=$db->query("select id,name,created_at from {$tx}payment_methods");
		$data=[];
	...
	public static function pagination($page=1,$perpage=10,$criteria=""){
		global $db,$tx;
		$top=($page-1)*$perpage;
		$result=$db->query("select id,name,created_at from {$tx}payment_methods $criteria limit $top,$perpage");
		$data=[];
		while($paymentmethod=$result->fetch_object()){
			$data[]=$paymentmethod;
		}
	...
	public static function find($id){
		global $db,$tx;
		$result =$db->query("select id,name,created_at from {$tx}payment_methods where id='$id'");
...
";
	...
	static function html_table($page = 1,$perpage = 10,$criteria="",$action=true){
		...
		while($paymentmethod=$result->fetch_object()){
			...
			$html.="<tr><td>$paymentmethod->id</td><td>$paymentmethod->name</td><td>$paymentmethod->created_at</td> $action_buttons</tr>";
		}
		$html.="</table>";
	...
	static function html_row_details($id){
		...
		$html.="<tr><th>Created At</th><td>$paymentmethod->created_at</td></tr>";
