...
class WorkSchedule extends Model implements JsonSerializable{
	...
	public function set($id,$staff_id,$start_time,$end_time,$created_at){
		$this->id=$id;
	...
	public function save(){
		global $db,$tx;
		$db->query("insert into {$tx}work_schedules(staff_id,start_time,end_time,created_at)values('$this->staff_id','$this->start_time','$this->end_time','$this->created_at')");
		return $db->insert_id;
	}
	public function update(){
		global $db,$tx;
		$db->query("update {$tx}work_schedules set staff_id='$this->staff_id',start_time='$this->start_time',end_time='$this->end_time',created_at='$this->created_at' where id='$this->id'");
	}
	...
	public static function all(){
		global $db,$tx;
		$result=$db->query("select id,staff_id,start_time,end_time,created_at from {$tx}work_schedules");
		$data=[];
	...
	public static function pagination($page=1,$perpage=10,$criteria=""){
		global $db,$tx;
		$top=($page-1)*$perpage;
		$result=$db->query("select id,staff_id,start_time,end_time,created_at from {$tx}work_schedules $criteria limit $top,$perpage");
		$data=[];
		while($workschedule=$result->fetch_object()){
			$data[]=$workschedule;
		}
	...
	public static function find($id){
		global $db,$tx;
		$result =$db->query("select id,staff_id,start_time,end_time,created_at from {$tx}work_schedules where id='$id'");
